...
class ItchUdpListener {
public:
    explicit ItchUdpListener(uint16_t udpPort, Callback callback, int cpuCore = -1)
        : _udpPort(udpPort),
          _cpuCore(cpuCore),
          ...
          _isRunning(false)
    {
        if (_cpuCore < -1) throw std::invalid_argument("Invalid CPU core index");
    }
